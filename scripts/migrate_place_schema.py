    ...
    for place in item.get("Place_Mentions", []):
        ...
        if "route" in place and isinstance(place["route"], list):
            for stop in place["route"]:
                ...
                if "map_urls" not in stop and "latitude" in stop and "longitude" in stop:
                    lat = stop["latitude"]
                    lon = stop["longitude"]
                    stop["map_urls"] = {
                        "google_maps": f"https://www.google.com/maps?q={lat},{lon}",
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Migrate place JSON files to schema v2.0.0")
